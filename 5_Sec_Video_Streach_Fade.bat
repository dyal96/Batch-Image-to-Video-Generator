...
"%~dp0ffmpeg.exe" -y -loop 1 -t %duration% -i "img!j!.jpg" -vf "scale=%resolution%,setsar=1,format=yuv420p" -r %fps% -c:v libx264 -preset veryfast -t %duration% "clip!j!.mp4"
if exist "clip!j!.mp4" (
...
cmd /V /C ""%~dp0ffmpeg.exe"!inputs! -filter_complex "!filtergraph!" !final_map! -r %fps% -c:v libx264 -preset veryfast -movflags +faststart "%~dp0output.mp4""
