...
for %%F in (%*) do (
    ...
    if /I "!ext!"==".pdf" (
        echo [PDF] Converting: %%~nxF
        :: -colorspace sRGB fixes the CMYK color issue
        "%magick_exe%" -density 300 "%%~fF" -colorspace sRGB -quality %jpeg_quality% "%workdir%\pdf_page_%%d.jpg"
...
:: Fix math calculation with quotes
